document.addEventListener('DOMContentLoaded', () => {
    ...
    function startPreviewLoop() {
        function loop() {
            ...
            // Apply Filter
            overlayCtx.filter = filterSelect.value === 'none' ? 'none' : filterSelect.value;
            ...
            // Draw Frame Overlay
            drawFrame(overlayCtx, overlayCanvas.width, overlayCanvas.height, currentFrame);
    ...
    function drawFrame(ctx, w, h, frameId) {
        ...
        switch(frameId) {
            ...
            case 'cat':
                ...
                // Left whiskers
                ctx.beginPath();
                ctx.moveTo(faceX - 45, faceY - 15); ctx.lineTo(faceX - 75, faceY - 20);
                ctx.moveTo(faceX - 45, faceY - 10); ctx.lineTo(faceX - 75, faceY - 10);
                ctx.stroke();
                // Right whiskers
                ctx.beginPath();
                ctx.moveTo(faceX + 45, faceY - 15); ctx.lineTo(faceX + 75, faceY - 20);
            ...
            case 'star':
                ...
                drawStar(p, p); drawStar(w-p, p); 
                drawStar(p, h-p); drawStar(w-p, h-p);
                drawStar(w/2, p/2); drawStar(w/2, h-p/2);
                break;
    ...
    captureBtn.addEventListener('click', async () => {
        ...
        const numPhotos = (currentLayout === '1x1') ? 1 : (currentLayout === '2x1' ? 2 : 4);
        capturedPhotos = [];
    ...
    function getLayoutPos(i, tw, th, p, layout) {
        ...
        if (layout === '1x1') { x=p; y=p; w=aw; h=ah; }
